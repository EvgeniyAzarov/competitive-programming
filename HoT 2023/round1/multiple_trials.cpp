    ...
    for (int i = 0; i < 20; i++)
    {
        ...
        do {
            ...
            for (auto &core_info : total) 
            {
                ...
                vector<int> ordered_msgs = determine_msgs_order(core_info.second, core_cur_msgs[c][max_set_type], messages);

                for (int user : ordered_msgs)
                {
                    MSG msg = messages[user].front();
                    messages[user].pop();
                    cur_distribution[c].push_back(msg);
                    core_info.second += msg.time;

                    if (core[msg.user] == -1) core[msg.user] = c;
        ...
        int score = compute_score(cur_distribution);
