...
vector<vector<MSG> > strategy2(map<int, queue<MSG> > messages, int m, int global_deadline)
{
    vector<vector<MSG> > distribution(m);
    vector<int> core(10001, -1);
    vector<int> total(m);
    ...
    do {
        ...
        for (int c = 0; c < m; c++)
        {
            ...
            for (size_t i = 1; i < 201; i++)
            {
                int local_score = core_cur_msgs[c][i].size();

                // Take the capability into account
                for (const auto &it : core_cur_msgs[c][i])
                {
                    MSG msg = messages[it].front();
                    if (total[c] + msg.time <= min(msg.deadline, global_deadline)) local_score++;
                }
                

                if (local_score > max_set)
                {
                    max_set=local_score;
                    max_set_type=i;
                }
...
int main()
{
    ...
    auto distribution2 = strategy2(messages, m, global_deadline);
